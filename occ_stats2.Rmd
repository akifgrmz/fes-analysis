...
plot_rmse<-filter(rmse,  Norm == TRUE, Error_Type=="RMSE", Error!=0) %>% filter(!(MVC_Stim==0 & Filt!="Unfilt"))%>%
...
plot_rmse_summary <- plot_rmse %>%
  group_by(Filt, Effort_Type, MVC_Voli_Cat, MVC_Stim_Cat) %>%
  summarise(
...
ggplot(plot_rmse_summary, aes(x = Filt  , y = 100*mean_error, fill = MVC_Stim_Cat)) +
    geom_col(position = "dodge") +     #geom_smooth(method = "lm")+ 
    geom_errorbar(aes(ymin = 100*(mean_error - sd_error), ymax = 100*(mean_error + sd_error)),
                position = position_dodge(0.9), width = 0.25) +    
  facet_wrap(Effort_Type~MVC_Voli_Cat)+
    #ylim(-10,200)
    labs(x = "Filter ",
    y = "NRMSE (% MVC MAV)",
    title = "NRMSE ",
