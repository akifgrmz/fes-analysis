...
plot_rmse_summary_test <- plot_rmse %>%
  group_by(Filt, Effort_Type, MVC_Voli_Cat,Test, MVC_Stim_Cat) %>%
  summarise(
    mean_error = mean(Error),
    sd_error = sd(Error),
    n = n(),  # Get the sample size
...
g1<-ggplot(f_tibble, aes(x = Filt  , y = mean_error, fill = MVC_Voli_Cat)) +
  ...
  theme(
    ...
    label = "p",  # Display exact p-values
...
#pdf("rmse_results.pdf", width = 6, height = 6)  # specify dimensions in inches
...
# Filter for specific Effort_Type: "Hand"
h_tibble <- filter(plot_rmse, Effort_Type == "Hand")

# Plot for Hand Experiments
g2 <- ggplot(h_tibble, aes(x = Filt, y = Error, fill = MVC_Voli_Cat)) +
  geom_boxplot(position = position_dodge(0.9)) +  # Use boxplots for raw data
  labs(
    x = "Filter",
    y = "Normalized Root Mean Squared Error",
    title = "Finger Extension Experiments",
    subtitle = sprintf("%d participants", length(unique(h_tibble$Test)))
  ) +
  theme_minimal() +
  theme(
    strip.background = element_blank(),
    legend.position = c(0.23, 0.48),
...
# Filter for specific Effort_Type: "Force"
f_tibble <- filter(plot_rmse, Effort_Type == "Force")

# Plot for Force Experiments
g1 <- ggplot(f_tibble, aes(x = Filt, y = Error, fill = MVC_Voli_Cat)) +
  geom_boxplot(position = position_dodge(0.9)) +  # Use boxplots for raw data
  labs(
    x = "Filter",
    y = "Normalized Root Mean Squared Error",
    title = "Isometric Force Experiments",
    subtitle = sprintf("%d participants", length(unique(f_tibble$Test)))
  ) +
  theme_minimal() +
  theme(
    strip.background = element_blank(),
  ...
  stat_compare_means(
    data = f_tibble,  # Use raw data for comparisons
    aes(x = Filt, y = Error),  # Specify x and y axes for p-values
    comparisons = list(c("Ref", "GS"), c("Ref", "Comb"), c("Ref", "Unfilt")),  # Define comparisons
    label = "p.signif",  # Show exact p-values
    method = "wilcox.test"  # Wilcoxon test for comparisons
  )

# Display both plots side by side
library(patchwork)  # For combining plots
#pdf("rmse_pval.pdf", width = 6, height = 6)  # specify dimensions in inches
...
f_tibble<-filter(plot_rmse_summary_test, Effort_Type == "Force")
parti_tibble<-filter(plot_rmse, Effort_Type == "Force")

p1<-ggplot(f_tibble, aes(x = Filt  , y = mean_error, fill = MVC_Voli_Cat)) +
    geom_col(position = "dodge") +
    geom_errorbar(aes(ymin = (mean_error - conf_int), ymax = (mean_error + conf_int)),
                position = position_dodge(0.9), width = 0.25) +    
  facet_grid(MVC_Stim_Cat~Test)+ 
  #coord_cartesian(ylim=c(0,0.5)) + 
    labs(x = "Filter ", 
    y = "NRMSE (% MVC MAV)",
    title = "NRMSE ",
...
h_tibble<-filter(plot_rmse_summary_test, Effort_Type == "Hand")
parti_tibble<-filter(plot_rmse, Effort_Type == "Hand")
p2<-ggplot(h_tibble, aes(x = Filt  , y = mean_error, fill = MVC_Voli_Cat)) +
    geom_col(position = "dodge") +
    geom_errorbar(aes(ymin = (mean_error - conf_int), ymax = (mean_error + conf_int)),
                position = position_dodge(0.9), width = 0.25) +    
  facet_grid(MVC_Stim_Cat~Test)+ 
  ...
  theme(
    strip.background = element_blank(),
    #strip.text = element_text(face = "bold"),
    legend.position = c(0.23, 0.46),  # Position legend inside the plot (top-right corner)
    legend.background = element_rect(fill = alpha("white", 0.6))  # Semi-transparent background for readability
  ) + 
    stat_compare_means(data = filter(plot_rmse_summary, Effort_Type == "Hand"), aes(), 
                     comparisons = list(c("Ref", "GS"), c("Ref", "Comb"), c("Ref", "Unfilt")),
                     label = "p.signif",
                     method = "wilcox.test")
#pdf("rmse_tests.pdf", width = 12, height = 8)  # specify dimensions in inches
