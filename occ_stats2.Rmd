...
occraw<- read_csv("occlusion_v3.csv",show_col_types = FALSE)
...
plot_tibble<-filter(occ, MVC_Stim!="Stim_0",Tau=="3*tau",Filt=='Unfilt')


ggplot(plot_tibble, aes(x =  v_mvc , y = vprime_mvc , color = MVC_Stim)) +
    geom_point() +
...
ggplot(plot_tibble, aes(x =  v_mvc , y = Occ_mvc , color = MVC_Stim)) +
    geom_point() +
    geom_smooth(method = "lm")+
    facet_wrap(~Feat)
...
ggplot(plot_tibble, aes(x =  MVC_Voli, y = MVC_Stim, fill = Occ_mvc)) +
    geom_tile() +
    geom_smooth(method = "lm")+
    facet_wrap(~Feat)
