...
droppedocc<- read_csv("dropped_occ2.csv",show_col_types = FALSE)

occraw<- read_csv("occlusion_v5.csv",show_col_types = FALSE)
err_raw<- read_csv("occ_est_error2.csv",show_col_types = FALSE)
err_raw
droppedocc
occraw
coefraw
...
ggplot(plot_tibble, aes(x =  v_mvc , y = vprime_mvc , color = MVC_Stim_Cat)) +
    ...
    subtitle = "2 able-body participants")

lm_tibble<-filter(occ, MVC_Stim_Cat!="Stim_0",Tau=="3*tau",Filt=='Unfilt',Feat=='Hand')

lm_tibble <- lm_tibble %>%
  mutate(Test = relevel(Test, ref = "jul9_24"))
...
```{r }
simp_tipple<-filter(occ, MVC_Stim_Cat!="Stim_0",Tau=="3*tau",Filt=='Unfilt',Feat=='Force')%>%select(Occ_mvc,Occ_Dropped_mvc,MVC_Stim_Cat,MVC_Voli,MVC_Stim,v_mvc,Feat)
...
plotibble<-filter(droppedocc, NumofDropped !='4_Drops')
formula <- y ~ poly(x, 1, raw = TRUE)
my.format <-"b[0]~`=`~%.3g*\", \"*b[1]~`=`~%.3g"
p1<-ggplot(plotibble, aes(x =  VoliMVC , y = MAV, color = MAV_Type))+
  geom_point() +
...
#lm_tibble<-filter(plotibble, MAV_Type =='Dropped')
  ...
  m1 <- lm(100*MAV ~ VoliMVC+StimMVC+MAV_Type, data = plotibble) 
tidy(m1,conf.int = TRUE ,conf.level =0.95) %>% knitr::kable(digits = 3)
