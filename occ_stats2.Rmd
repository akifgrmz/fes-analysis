...
occ<-occ%>%filter(Test!="mar7",Test!="mar16",Test!="feb27",Test!="may19")
...
ggplot(plotibble, aes(x =  VoliMVC , y = Dropped_MAV, color = StimMVC))+
  ...
  geom_segment(aes(x = 10, y = .1, xend = 40, yend = .4, color='No Occlusion Level'))+
      facet_wrap(~Test)
...
ggplot(plotibble, aes(x =  VoliMVC , y = Occ_MAV*100 , color = StimMVC))+
  geom_point() +
  geom_smooth(method = "lm",level=0.90)+
      facet_wrap(~Test)
