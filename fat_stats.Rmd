...
ggplot(data = fat_byframe, aes(x = R_sqr, fill = Test)) + geom_histogram(color = "white", bins = 20) +
labs(title = "", x = "%MVC") + guides(fill = FALSE) + facet_grid( ~ Test)
...
p2<-ggplot(filter(fat_byframe_data, EMG=='vEMG', Filt=='Unfilt', Feat=='MedFreq'),
           aes(x = Trial, y = Feat_Val, color = Test)) +
  geom_point(stat = "identity", position=position_dodge()) +
  geom_smooth(method = "lm", se=F)+
  facet_grid( ~ Segment)+
...
p3<-ggplot(filter(fat_byframe_data, EMG=='vEMG', Filt=='GS',Feat=='MedFreq')
           ,aes(x = Trial, y = Force, color = Test)) +
  geom_point(stat = "identity", position=position_dodge()) +
  geom_smooth(method = "lm", se=F)+
  labs(x = "Trials",
  y = "Force(N)",
