...
TestString<-sprintf("Tests: %s",TestString)

#pdf("figures/4dropped2.pdf")

plot_df<-filter(df,Test==tests,!is.na(Drp_Order))

ggplot(plot_df, aes(x = Frame, y = Norm_Val, color = Drp_Order)) +
    geom_point() +
    geom_line(data= filter(df,Test==tests,Repeat=="Rep_1",MVC_Stim!="Stim_0%",Filt_Type==c("Unfilt")), aes(x = Frame, y = Target*3, color = "Target (a.u.)")) +
