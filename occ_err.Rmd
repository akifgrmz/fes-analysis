...
err_raw<-err%>%filter(Test!="mar7",Test!="mar16",Test!="feb27",Test!="may19")
...
p2<-ggplot(occ_err, aes(x =  VoliMVC , y = Effort_err_Amp_Occ_Indv , color = MVC_Stim_Cat)) +
    geom_point() +
    geom_smooth(method = "lm")+ 
    geom_segment(aes(x = 9, y = 0, xend = 41, yend = 0, color='Zero Error'))+
    ylim(-55, 25)+
...
p3<-ggplot(occ_err, aes(x =  VoliMVC , y = Effort_err_Amp_Occ , color = MVC_Stim_Cat)) +
    geom_point() +
    geom_smooth(method = "lm")+ 
    geom_segment(aes(x = 9, y = 0, xend = 41, yend = 0, color='Zero Error'))+
    ylim(-55, 25)+
...
mean_err<-occ_err %>% group_by(MVC_Voli_Cat) %>% summarise( "Mean"= round(mean(Effort_err_Amp),2), "RMS"=round(sqrt(mean(Effort_err_Amp^2)),2),"Est"='Before Correction') 
mean_err<-mean_err%>%add_row(occ_err %>% group_by(MVC_Voli_Cat) %>% summarise( "Mean"= round(mean(Effort_err_Amp_Occ_Indv),2), "RMS"=round(sqrt(mean(Effort_err_Amp_Occ_Indv^2)),2),"Est"='Individualized Correction') )
mean_err<-mean_err%>%add_row(occ_err %>% group_by(MVC_Voli_Cat) %>% summarise( "Mean"= round(mean(Effort_err_Amp_Occ),2), "RMS"=round(sqrt(mean(Effort_err_Amp_Occ_Indv^2)),2),"Est"='After Correction') )
