...
mean_occ_mvc <- filter(occ,Tau==selected_tau,MVC_Stim!="Stim_0") %>% group_by( MVC_Voli, MVC_Stim) %>% summarise( Mean_F_vprime=round(mean(F_vprime_mvc),3), Std_F_vprime=round(sd(F_vprime_mvc),3), Mean_F_occ=round(mean(F_occ_mvc),3), Std_F_occ=round(sd(F_occ_mvc),3), n = n())
mean_occ_mvc
