...
date: "2023-01-27"
desc: This file is created for analysis of dropped frames for EMBC2023
# theme: darkly
---
...
dt_mn <- aggregate(dt_means$Filt_Mean_Val, by = list( Test = dt_means$Test, Filt_Type = dt_means$Filt_Type, MVC_Voli = dt_means$MVC_Voli, MVC_Stim =
                                                        dt_means$MVC_Stim,Dropped = dt_means$Dropped), mean)
dt_mn
ggplot(filter(dt_means,Filt_Type!='Dropped'),aes(x = Test, y = percent_diff, fill = Filt_Type)) + 
  geom_bar(stat = "identity", position=position_dodge()) +
  facet_wrap(Dropped~MVC_Voli)+
  #geom_errorbar(aes(ymin=mean(percent_diff)-sd(percent_diff), ymax=mean(percent_diff)+sd(percent_diff)), width=.2,position=position_dodge(.9)) +
  labs(x = "Filter Types",
       y = "Mean Percent Difference",
...
```{r plotting a test with boxplots }

plot<-filter(dt,Test=='jan12')%>% select( c(Frame_Val,Norm_Val, Target, Filt_Type, Dropped,Frame,Test,MVC_Voli,MVC_Stim) )  
