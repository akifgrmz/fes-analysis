...
df %>% group_by(Test) %>% summarise("Pearson r" = round(cor(TimeConst, MVC_Percent, use="complete"),2), "Spearman r" = round(cor(TimeConst, MVC_Percent, method="spearman",use="complete"),2))
...
label = paste("Pearson r = ", signif(cor(df$MVC_Percent, df$TimeConst),3))) +
  annotate("text", x = 20, y = 0.6, col = "red", size = 5,
