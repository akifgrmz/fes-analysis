...
p2 <- ggplot(plot_tibble, aes(x = MVC_Voli, y = Occ_mvc, color = MVC_Stim_Cat)) +
  ...
  geom_smooth(method = "lm", size = 2, se = FALSE) +
  facet_grid(~Feat) +
  coord_cartesian(ylim = c(-10, 87)) +  # Extended upper limit for annotations
  labs(
    x = expression("Volitional Hand Opening: V (% MFE)"),
...
# Filter data for Force effort type only
plot_tibble_force <- filter(occ, 
                            MVC_Stim_Cat != "Stim_0",
                            Tau == "3*tau",
                            Filt == 'Unfilt',
                            Feat != 'Amp_Modul',
...
NumofTest <- length(unique(plot_tibble_force$Test))

# Recode feature names
plot_tibble_force <- plot_tibble_force %>%
  mutate(Feat = recode(Feat, 'EffortMea' = 'Force'))

# Create annotation data for Force plots
annotation_data1_force <- data.frame(
  Feat = c("Force", "MAV"),
  x = 10, 
  y = 95, 
  label = c("(a)", "(b)")
...
annotation_data2_force <- data.frame(
  Feat = c("Force", "MAV"),
  x = 10, 
  y = 75, 
  label = c("(c)", "(d)")
...
# Calculate statistics for both response variables
stats_data1_force <- get_slope_stats(plot_tibble_force, "vprime_mvc")
stats_data2_force <- get_slope_stats(plot_tibble_force, "Occ_mvc")

# Create p-value annotations
create_pval_annotation <- function(stats_data) {
  stats_data %>%
    arrange(MVC_Stim_Cat) %>%
    group_by(Feat) %>%
    summarise(
      pval_text = paste(
        paste(sprintf("%s (n=%d): s=%.3f, %s", 
                      MVC_Stim_Cat, 
                      n, 
                      slope,
                      p_label), 
...
pval_annotation1_force <- create_pval_annotation(stats_data1_force)
pval_annotation2_force <- create_pval_annotation(stats_data2_force) %>%
  mutate(y = 85)

# Get unique stimulation categories and create color palette
stim_cats_force <- sort(unique(plot_tibble_force$MVC_Stim_Cat))
n_stim_force <- length(stim_cats_force)

# Define colors for all stimulation levels
stim_colors <- c("Stim_10" = "#00BA38", 
                 "Stim_20" = "#F8766D", 
                 "Stim_30" = "#619CFF",
                 "Stim_40" = "#C77CFF")
color_values_force <- stim_colors[stim_cats_force]
color_values_force["No Occlusion Level"] <- "black"

# Create dynamic labels based on actual stimulation levels
stim_labels <- gsub("Stim_", "", stim_cats_force)
stim_labels <- paste0(stim_labels, "% Stim")
legend_labels_force <- c('No occlusion level', stim_labels)

# Plot 1: Maintained Effort (V') - Force
p1_force <- ggplot(plot_tibble_force, aes(x = MVC_Voli, y = vprime_mvc, color = MVC_Stim_Cat)) +
  geom_point(size = 2) +
  geom_smooth(method = "lm", size = 2, se = FALSE) +
  ...
  labs(
    x = expression("Volitional Force: V (% MVC)"),
    y = expression("Maintained Effort: V' (% MVC)")
  ) +
  theme_minimal(base_size = 18) +
  theme(
    strip.background = element_blank(),
  ...
  scale_color_manual(
    name = expression(""),
    values = color_values_force,
    breaks = c('No Occlusion Level', stim_cats_force),
    labels = legend_labels_force
  ) +
  geom_text(
    data = annotation_data1_force, 
    aes(x = x, y = y, label = label), 
    size = 7, color = "black", fontface = "bold"
  ) +
  geom_label(
    data = pval_annotation1_force,
    aes(x = x, y = y, label = pval_text),
    inherit.aes = FALSE,
    hjust = 0,
    vjust = 1,
...
# Plot 2: Occluded Effort - Force
p2_force <- ggplot(plot_tibble_force, aes(x = MVC_Voli, y = Occ_mvc, color = MVC_Stim_Cat)) +
  geom_point(size = 2) +
  geom_smooth(method = "lm", size = 2, se = FALSE) +
  facet_grid(~ Feat) +
  coord_cartesian(ylim = c(-10, 87)) +
  labs(
    x = expression("Volitional Force: V (% MVC)"),
  ...
  scale_color_manual(
    values = stim_colors[stim_cats_force]
  ) +
  geom_text(
    data = annotation_data2_force,
    aes(x = x, y = y, label = label), 
    size = 7, color = "black", fontface = "bold"
  ) +
  geom_label(
    data = pval_annotation2_force,
    aes(x = x, y = y, label = pval_text),
    inherit.aes = FALSE,
    hjust = 0,
    vjust = 1,
...
# Combine Force plots
combined_plot_force <- p1_force / p2_force
combined_plot_force

# Save the Force figure
ggsave("figures/force_occ.pdf", 
       plot = combined_plot_force, 
       width = 8, 
       height = 10, 
       dpi = 300)
```
